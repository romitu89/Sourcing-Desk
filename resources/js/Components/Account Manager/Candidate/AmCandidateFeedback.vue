                ...
                <div class="col-sm-12">
                  <div class="card">
                    ...
                    <div class="card-block">
                      <form id="main" @submit.prevent="feedback()" novalidate>
  ...
  export default {
    ...
    methods: {
      async accepted(data) {
        try {
          if (data.status) {
            ...
            const response = await axios.post(route('candidate-update', {
      ...
      populateInput(result) {
        ...
        this.response = []; // clear the response array
      ...
      search() {
        if (this.form.query.length) {
          ...
          axios
            .get(route("candidate-search", { query: this.form.query }))
            .then((response) => {
      ...
      feedback() {
        if (this.form.query.length) {
          router.post(route("candidate-feedback"), this.form, {
            preserveScroll: true,
            onSuccess: (data) => {
              // this.resetForm();
            },
